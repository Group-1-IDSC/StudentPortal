...
public partial class DashboardPage : ContentPage
{
    ...
    {
        Dispatcher.Dispatch(async () => // Use Dispatch to get onto the UI thread initially
        {
            Dispatcher.StartTimer(TimeSpan.FromSeconds(1), () =>
            {
                // Access the Dispatcher of the current window
                if (Application.Current?.MainPage?.Window != null)
                {
                    Application.Current.MainPage.Window.Dispatcher.Dispatch(() =>
                    {
                ...
                return false; // Stop the timer if the window is no longer available
            });
        });
    }
